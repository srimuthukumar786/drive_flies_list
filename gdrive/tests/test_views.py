...
def test_list_drive_files_missing_param(client):
    response = client.get(reverse("gdrive_list"))  # <-- updated
...
def test_list_drive_files_success(mock_list_files, client):
    mock_list_files.return_value = [{"id": "123", "name": "test.txt"}]
    response = client.get(reverse("gdrive_list") + "?folder_id=abc")  # <-- updated
    assert response.status_code == 200
